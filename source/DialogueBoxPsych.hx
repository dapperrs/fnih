...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	var box:FlxSprite;
	var slidesBG:FlxSprite;
	var textToType:String = '';
	var slides:Float = 0;
	var yaornah:Bool = false;
	...
	function startNextDialog():Void
	{
		...
		slidesBG = new FlxSprite().loadGraphic(Paths.image(curDialogue.slidePath));
		slidesBG.antialiasing = ClientPrefs.globalAntialiasing;
		slidesBG.setGraphicSize(Std.int(slidesBG.width * 1.1));
		slidesBG.screenCenter();

		//all this looks so dumb but it works
		remove(slidesBG);
		remove(box);
		remove(char);
		remove(daText);
