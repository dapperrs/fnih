...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		bg = new FlxSprite();
		bg.frames = Paths.getSparrowAtlas('freeplaymenu/FPlayBG');
		bg.animation.addByPrefix('idle', "freeplayBGH", 24);
		bg.animation.play('idle');
		bg.antialiasing = ClientPrefs.globalAntialiasing;
		add(bg);

		drawing = new FlxSprite(500, 100).loadGraphic(Paths.image('freeplaymenu/hahadrawingew'));
		drawing.antialiasing = ClientPrefs.globalAntialiasing;
		drawing.setGraphicSize(Std.int(drawing.width * 0.8));
		drawing.alpha = 0.4;
		add(drawing);
		...
		drawing.color = songs[curSelected].color;
	...
	function changeSelection(change:Int = 0)
	{
		...
		if(newColor != intendedColor) {
			...
			colorTween = FlxTween.color(drawing, 1, drawing.color, intendedColor, {
				onComplete: function(twn:FlxTween) {
					colorTween = null;
				}
			});
