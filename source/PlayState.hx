...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['LMAO You Suck!', 0.2], //From 0% to 19%
		['CRINGE', 0.4], //From 20% to 39%
		['Bad', 0.5], //From 40% to 49%
		['Bruh.', 0.6], //From 50% to 59%
		['Moe', 0.69], //From 60% to 68%
		['Cool', 0.7], //69%
		['Good', 0.8], //From 70% to 79%
		['Great', 0.9], //From 80% to 89%
		['Sick!', 1], //From 90% to 99%
		['Sheeeesh!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public var timerPhoto:FlxTimer;

	public var blimpMove:FlxTween;
	...
	public static var clickedAlready:Bool = false;
	...
	var butt:FlxTypedGroup<FlxSprite>;
	var timerCountdown:FlxTypedGroup<FlxText>;

	var blimp:BGSprite;
	var camSpriteLol:BGSprite;
	var camSpriteLol2:BGSprite;
	var cool:BGSprite;

	var henchmanFront:BGSprite;

	var henchmanSolo:BGSprite;
	...
	var henchmen:BGSprite;
	var hasNotFinished:Bool = true;

	var limoKillingState:Int = 0;
	var limo:BGSprite;
	var buttButton:FlxButton;
	var limoMetalPole:BGSprite;
	var limoLight:BGSprite;
	var curThing:Int;

	var isPurpleTapped:Bool = false;
	var isBlueTapped:Bool = false;
	var isGreenTapped:Bool = false;
	var isRedTapped:Bool = false;
	//THESE SUCK
	//wait na i need them

	var curSelected:Int = 0;

	var pictureGuy:FlxSprite;
	var limoCorpse:BGSprite;
	var limoCorpseTwo:BGSprite;
	var bgLimo:BGSprite;
	var walkingGuy:BGSprite;
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'thorns':
					curStage = 'schoolEvil';
				case 'star' | 'cali' | 'hollywood' | 'guns':
					curStage = 'angeles'; 
				case 'hostage':
					curStage = 'angelesCloak'; 	
				case 'meeting':
					curStage = 'black'; 								
				default:
					curStage = 'stage';
		...
		switch (curStage)
		{
			...
			case 'black': //meeting
			    var black:FlxSprite = new FlxSprite().makeGraphic(Std.int(FlxG.width * 3), Std.int(FlxG.height * 3), FlxColor.BLACK);
				add(black);
			case 'angeles': //Week 7
				var bg:BGSprite = new BGSprite('angeles/DaSky', -950, -650, 0.1, 0.1);
				bg.setGraphicSize(Std.int(bg.width * 1.8));
				add(bg);

				if(!ClientPrefs.lowQuality) {
				blimp = new BGSprite('angeles/blimp', 1450, -20, 0.1, 0.1);
				blimp.setGraphicSize(Std.int(blimp.width * 1.4));
				add(blimp); }
	
				var mountains:BGSprite = new BGSprite('angeles/hollyWoodMountains', -1100, 130, 0.1, 0.1);
				mountains.setGraphicSize(Std.int(mountains.width * 1));
				add(mountains);

				var coollightse:BGSprite = new BGSprite('angeles/lights/backgroundLightThingamabobs', 600, -450, 0.1, 0.1, ['lightsBG'], true);
				add(coollightse); 

				var buildings:BGSprite = new BGSprite('angeles/daBuilds', -300, 350, 0.1, 0.1, ['lightsBuilding'], true);
				buildings.setGraphicSize(Std.int(buildings.width * 1.35));
				add(buildings); 

				var coollights:BGSprite = new BGSprite('angeles/lights/backgroundLightThingamabobs', -400, -400, 0.1, 0.1, ['lightsBG'], true);
				add(coollights); 

				var balcony:BGSprite = new BGSprite('angeles/balcony', -300, 900);
				balcony.setGraphicSize(Std.int(balcony.width * 1.5));
				add(balcony);

				henchmen = new BGSprite('angeles/henchmanGroup', 900, 450, ['henchmenGroup'], true);
				henchmen.setGraphicSize(Std.int(henchmen.width * 1.5));
				add(henchmen);

				if(!ClientPrefs.lowQuality)
				{
					henchmanSolo = new BGSprite('angeles/henchmanSolo', -400, 550, ['henchmanSolo'], true);
					henchmanSolo.flipX = true;
					henchmanSolo.color = FlxColor.GRAY;
					henchmanSolo.setGraphicSize(Std.int(henchmanSolo.width * 3));
				}	


				camSpriteLol = new BGSprite('angeles/camera', -900, 650);
				camSpriteLol.setGraphicSize(Std.int(camSpriteLol.width * 2.5));
				
				camSpriteLol2 = new BGSprite('angeles/camera', 1700, 1230);
				camSpriteLol2.flipX = true;
				camSpriteLol2.color = FlxColor.GRAY;
				camSpriteLol2.setGraphicSize(Std.int(camSpriteLol.width * 3.4));

				cool = new BGSprite('angelesCloak/niceLight', -200, 350, 0.1, 0.1);
				cool.setGraphicSize(Std.int(cool.width * 1.2));	
				cool.screenCenter();

				if(!ClientPrefs.lowQuality)
					{
						henchmanFront = new BGSprite('angeles/henchmanFront', 1450, 750, ['frontDude'], true);
						henchmanFront.setGraphicSize(Std.int(henchmanFront.width * 1.3));
					}	


			case 'angelesCloak': //Week 7
				var bg:BGSprite = new BGSprite('angeles/DaSky', -950, -650, 0.1, 0.1);
				bg.setGraphicSize(Std.int(bg.width * 1.8));
				add(bg);

				blimp = new BGSprite('angeles/blimp', 1350, -20, 0.1, 0.1);
				blimp.setGraphicSize(Std.int(blimp.width * 1.4));
				add(blimp);
				...
				henchmen = new BGSprite('angeles/henchmenDeadGroup', 950, 800, ['henchmenDead'], true);
				henchmen.setGraphicSize(Std.int(henchmen.width * 1.7));
				add(henchmen);
				...
				CoolUtil.precacheSound('cocked'); //hahah cok get it
				
				GameOverSubstate.characterName = 'bf-cloak';
		...
		butt = new FlxTypedGroup<FlxSprite>();
		add(butt); // dumb button lol
		...
		switch(curStage)
		{
			...
			case 'angelesCloak':
				...
				boyfriend.color = FlxColor.GRAY;
				dad.color = FlxColor.GRAY;
				gf.color = FlxColor.GRAY;
			case 'angeles':
				boyfriend.color = FlxColor.GRAY;
				dad.color = FlxColor.GRAY;
				gf.color = FlxColor.GRAY;
			case 'black':
				var evilTrail = new FlxTrail(dad, null, 4, 24, 0.3, 0.069); //nice
				insert(members.indexOf(dadGroup) - 1, evilTrail); //hahaha
		...
		pictureGuy = new FlxSprite(350, 200);
		pictureGuy.frames = Paths.getSparrowAtlas('angeles/photographer');
		pictureGuy.animation.addByPrefix('idle', 'photographe', 30, false);
		pictureGuy.setGraphicSize(Std.int(pictureGuy.width * 1.2));
		pictureGuy.visible = false;
		pictureGuy.antialiasing = false;
		add(pictureGuy);

		walkingPhoto = new FlxSprite(-700, 50);
		walkingPhoto.frames = Paths.getSparrowAtlas('angeles/photographerwalk');
		walkingPhoto.animation.addByPrefix('idle', 'walkingDudds', 24, false);
		walkingPhoto.setGraphicSize(Std.int(walkingPhoto.width * 1.2));
		walkingPhoto.visible = false;
		walkingPhoto.antialiasing = false;
		add(walkingPhoto);
		...
		doof.cameras = [camHUD];
		pictureGuy.cameras = [camHUD];
		walkingPhoto.cameras = [camHUD];
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'star' | 'cali' | 'hollywood' | 'hostage':
		...
		CoolUtil.precacheSound('gunDodge'); // bababa
	...
	// THESE ABOVE SUCK
	//nahhh ima bring em back

	//ARRAY WORKS MAYBE LETS SEE
	// O YEAH IT WORKS

	var herijgerjiogjre:Array<FlxSprite> = [];

	function doShots()
	{	
		if (hasNotFinished)
		{
			if (dad.curCharacter.startsWith('duo'))
			{ 
				dad.playAnim('pull', true);
				FlxG.sound.play(Paths.sound('cocked'));
		    }	
			else
				FlxG.sound.play(Paths.sound('cocked'));
		}	
		...
		for(i in herijgerjiogjre)
			remove(i);

		isPurpleTapped = false;
		isBlueTapped = false;
		isGreenTapped = false;
		isRedTapped = false;
		...
		for (i in 0...4)
		{
			//var daX = FlxG.random.int(-50,1250);
			//var daY = FlxG.random.int(100,600); // trying to make sure they dont collide, it'll fuck it up idk a fix

			var randomCoords = FlxG.random.int(1,4);

			var butte:FlxSprite = new FlxSprite();
			butte.frames = Paths.getSparrowAtlas('buttonThings/buttonThing');

			switch(i) // i hate this shit dude i suck at code
			{
				case 0: 		
					switch(randomCoords)
					{
				...
				case 3:
					...
					butte.color = 0xffDC565B;
			} 

			butte.animation.addByPrefix('doIt', 'hitHighlight', 24, false);
			butte.animation.addByPrefix('idle', 'hitSpawn', 24, false);
			butte.animation.addByPrefix('flash', 'flashOne', 24, false);
			butte.animation.addByPrefix('flashAgain', 'flashTwo', 24, false);
			butte.animation.addByPrefix('flashAgainAgain', 'flashThird', 24, false);
			butte.animation.play('idle', true);
			butte.setGraphicSize(Std.int(butte.width * 0.5));
			butte.updateHitbox();
			butte.scrollFactor.set();
			butt.add(butte);

			herijgerjiogjre.push(butte);	
		}	

		//trace(herijgerjiogjre);
	...
	{
		transition = new FlxSprite(-250, -500);
		transition.frames = Paths.getSparrowAtlas('songStartUps/transition' + curSong);
	...
	function startSong():Void
	{
		...
		var cool:String = WeekData.getWeekFileName();
		trace(cool);

		if (curSong == 'Hostage')
			{
				dad.playAnim('hint', true);
			}	
	...
	override public function update(elapsed:Float)
	{
		...
		{
			case 'Star':
				//blimpMove = FlxTween.tween(blimp, {x: 1400}, 93, {}); // make the blimp move, DUHHH
			case 'Cali':
				if (healthBar.percent < 50)
				{
					health -= 0.0007 * (elapsed / (1/40));
				}	
				//blimpMove = FlxTween.tween(blimp, {x: 1350}, 123, {}); // make the blimp move, DUHHH
			case 'Hostage':	
				/*if (healthBar.percent < 40)
					{
						health -= 0.0015 * (elapsed / (1/40));
					}	*/ 
				//make it easier	
				//blimpMove = FlxTween.tween(blimp, {x: 1300}, 186, {}); // i want this to move just a little
		...
		var justClickedOne:Bool = false;
		var finishhimoff = [];

		if (curStage == 'angelesCloak' || curStage == 'black') // ill admit, i did look at zardy's code :(
		{
			butt.forEach(function(spr:FlxSprite) // such a shit function thing lol 
				{	

					if (spr.animation.curAnim.finished && spr.animation.curAnim.name == 'idle')
					{
						spr.animation.play('flash', true);
					}	
					if (spr.animation.curAnim.finished && spr.animation.curAnim.name == 'flash')
					{
						spr.animation.play('flashAgain', true);
					}	
					if (spr.animation.curAnim.finished && spr.animation.curAnim.name == 'flashAgain')
					{
						spr.animation.play('flashAgainAgain', true);
					}	
					if (spr.animation.curAnim.finished && spr.animation.curAnim.name == 'flashAgainAgain')
					{
						health = 0;
			...
			butt.forEachAlive(function(spr:FlxSprite) // such a shit function thing lol // also idk if this foreachalive thing is nessescary // it works tho
				{	
					for (i in 0...herijgerjiogjre.length)
						{
							if (FlxG.mouse.overlaps(herijgerjiogjre[0]))
							{										
								if (!isPurpleTapped)
								{
									if (FlxG.mouse.justPressed)
									{	
										finishhimoff.push(herijgerjiogjre[0]);										
										isPurpleTapped = true;
										trace('purple');
										FlxG.sound.play(Paths.sound('gunDodge'));
										boyfriend.playAnim('dodge');
									}	
									FlxG.mouse.reset();	
								}	
								//FlxG.mouse.reset(); // boom fixed i think lol
								if (!isBlueTapped && isPurpleTapped)
								{
									if (FlxG.mouse.justPressed)
									{	
										finishhimoff.push(herijgerjiogjre[0]);										
								...
								if (isBlueTapped && isPurpleTapped)
								{
									if (!isGreenTapped && isBlueTapped && isPurpleTapped)
									{
										if (FlxG.mouse.justPressed)
										{	
											finishhimoff.push(herijgerjiogjre[0]);										
						...
						for (i in finishhimoff)	
							{
								if (herijgerjiogjre.contains(i)) // i think this is it
									{
										herijgerjiogjre.remove(i);
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'dead':
				var black:FlxSprite = new FlxSprite().makeGraphic(FlxG.width, 1236, FlxColor.BLACK);
				new FlxTimer().start(0.2, function(tmree:FlxTimer) 
					{
						add(black);
						remove(henchmen);
						var henchmen:BGSprite = new BGSprite('angeles/henchmenDeadGroup', 1050, 850, ['henchmenDead'], true);
						henchmen.setGraphicSize(Std.int(henchmen.width * 1.5));
						add(henchmen);
			    });
				new FlxTimer().start(2.3, function(tmree:FlxTimer) 
					{
						remove(black);
			    });
			...
			case 'blackLayer':
				var black:FlxSprite = new FlxSprite().makeGraphic(FlxG.width, 1236, FlxColor.BLACK);
				black.setGraphicSize(Std.int(black.width * 333));
				add(black);
			case 'Cheese': // should i do a seperate thing for downscroll and shiz? i think it's fine, actually nah ima do it
				switch(value1)
				{
					case 'left':
						walkingPhoto.flipX = false;
						pictureGuy.flipX = false;
					case 'right':
						walkingPhoto.flipX = true;
						pictureGuy.flipX = false;
					default:
						walkingPhoto.flipX = false;
						pictureGuy.flipX = false;
				}
				walkingPhoto.visible = true;
				walkingPhoto.animation.play('idle');

				var angleRandomizer = FlxG.random.int(0,10);

				if (ClientPrefs.downScroll) // good enough
				{
					pictureGuy.flipY = true;
					pictureGuy.y -= 520;
					pictureGuy.x += 200;
				}	
				else
					angleRandomizer = FlxG.random.int(0,10);
				switch (angleRandomizer)
				{
					case 0, 1:
						pictureGuy.x == 760;
						pictureGuy.angle = -5;
				...
				timerPhoto = new FlxTimer().start(5.2, function(tmree:FlxTimer) 
					{
						walkingPhoto.visible = false;
						pictureGuy.visible = true;
						pictureGuy.animation.play('idle');
			});
			    timerPhoto = new FlxTimer().start(9.5, function(tmre:FlxTimer) 
					{
						pictureGuy.x == 550;
				pictureGuy.visible = false;
			});
			...
			case 'Rise': //i hate this
				...
				{
					case 'bf':
						FlxTween.tween(boyfriend, {y: -300}, 10, {
							startDelay: Conductor.crochet * 0.001
						});	
					case 'dad':	
						FlxTween.tween(dad, {y: -300}, 10, {
							startDelay: Conductor.crochet * 0.001
						});	
				}
			case 'Shots':
				if(ClientPrefs.doge && !cpuControlled)
				{
					doShots();
				}	
				//hitEm(daXShallBe, 200, daNum);
		}
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['weekBruh_nomiss', 'ur_bad',
				'ur_good', 'hype', 'two_keys', 'toastie', 'crazy']);
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				{
					FlxG.sound.playMusic(Paths.music('coolMenu'));
					...
					if(!usedPractice) {
						...
						if (SONG.validScore)
						{
							Highscore.saveWeekScore('tutorial', campaignScore, storyDifficulty);
						}

						//FlxG.save.data.weekCompleted = StoryMenuState.weekCompleted;
						//FlxG.save.flush();

						//save em for update 2?
					}
					usedPractice = false;
				...
				else
				{
					...
					var hostageNext = (Paths.formatToSongPath(SONG.song) == "cali"); //lmao sorry
					if (hostageNext)
					{
						...
						FlxG.sound.play(Paths.sound('shots'));
					}
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName)) {
				...
				switch(achievementName)
				{
					case 'weekBruh_nomiss':
						...
						if (cool == 'bruh')
						{
							if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
								{
									if(achievementName == 'weekBruh_nomiss') unlock = true;	
								}	
						}							
					case 'ur_bad':
					...
					case 'crazy':
						if(Paths.formatToSongPath(SONG.song) == 'meeting' && !usedPractice) {
							unlock = true;
						}
				}
	...
	var curLightLOL:Int = 0;
	var curLightEventLOL:Int = 0;
}
